...
class UploadController
{
    public function uploadImage(Request $request, Response $response)
    {
        ...
        $mimeType = $fileInfo->buffer($uploadedFile->getStream()->getContents());
        
        $allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
        if (!in_array($mimeType, $allowedTypes)) {
            $response->getBody()->write(json_encode([
                'success' => false,
                'message' => 'Tipo de archivo no permitido. Solo se aceptan imágenes (JPEG, PNG, GIF, WebP).'
            ]));
            return $response->withStatus(400)->withHeader('Content-Type', 'application/json');
        }
        ...
        if (!is_dir($directory) || !is_writable($directory)) {
            $response->getBody()->write(json_encode([
                'success' => false,
                'message' => 'El directorio de destino para las subidas no existe o no tiene permisos de escritura.',
                'path' => $directory
            ]));
            return $response->withStatus(500)->withHeader('Content-Type', 'application/json');
        }
